...
def compute(s: str) -> int:
    ...
    to_be_removed = REQUIRED - (AVAILABLE - size_root)

    all_sizes = sorted([size_root, *[get_size(foo, str(dir)) for dir in dirs]])
    return sorted([s for s in all_sizes if s >= to_be_removed])[0]
